...
import { useLocation, getClients, getProducts, mutateExit } from '../../Hooks'
...
const ExitView = ({}) => {
  ...
  const { mutate: saveExit, isLoading: loadingExit } = mutateExit()
  ...
  useEffect(() => {
    var newValue = allClients.map(item => {
      ...
      >
        <MuiAppBar sx={{ position: 'relative' }}>
          <Toolbar>
            ...
            <Button color="inherit" disabled={loadingExit} onClick={save}>
              {'Crear'}
            </Button>
          </Toolbar>
        </MuiAppBar>
